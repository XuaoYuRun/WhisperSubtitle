...
class WhisperMinimalGUI(QMainWindow):
    ...
    def _update_params(self):
        """根据当前处理模式显示对应的 AI 参数（表格格式）"""
        def _row(name, value):
            return f"{name:28} │ {value}"
        ...
        if self.mode_cn.isChecked():
            text = (
                ...
                "┌─────────────────────────────┬──────────────────┐\n"
                + _row("language", "zh") + "\n"
                + _row("task", "transcribe") + "\n"
                + _row("beam_size", "5") + "\n"
                + _row("best_of", "5") + "\n"
                + _row("patience", "1.5") + "\n"
                + _row("length_penalty", "1.0") + "\n"
                + _row("temperature", "0.0") + "\n"
                + _row("compression_ratio_threshold", "2.4") + "\n"
                + _row("log_prob_threshold", "-1.0") + "\n"
                + _row("no_speech_threshold", "0.6") + "\n"
                + _row("condition_on_previous_text", "True") + "\n"
                + _row("word_timestamps", "False") + "\n"
                + _row("vad_filter", "True") + "\n"
                + _row("min_silence_duration_ms", "300") + "\n"
                "└─────────────────────────────┴──────────────────┘\n\n"
                "提示：中文标点自动转换，上下文连贯开启。"
            )
        elif self.mode_en_v1.isChecked():
            ...
            text = (
                ...
                + _row("length_penalty", "1.0") + "\n"
                + _row("temperature", "0.0") + "\n"
                + _row("compression_ratio_threshold", "2.0  ← 更严格") + "\n"
                + _row("log_prob_threshold", "-1.5  ← 更严格") + "\n"
                + _row("no_speech_threshold", "0.8  ← 更高") + "\n"
                + _row("condition_on_previous_text", "False  ← 关闭") + "\n"
                + _row("word_timestamps", "False") + "\n"
                + _row("vad_filter", "True") + "\n"
                + _row("min_silence_duration_ms", "500  ← 更长") + "\n"
                "└─────────────────────────────┴──────────────────┘\n\n"
